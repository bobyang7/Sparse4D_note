...
def topk(confidence, k, *inputs):
    ...
    return confidence, indices, outputs
...
class InstanceBank(nn.Module):
    ...
    def reset(self):
        self.cached_anchor_embedding = None
        self.cached_k_pred = None
    ...
    def get(self, batch_size, metas=None, dn_metas=None):
        ...
        if self.cached_anchor is not None and batch_size == self.cached_anchor.shape[0]:
            ...
            if self.anchor_handler is not None:
                ...
                )  # 计算历史的batch size的图像帧转换到当前帧的batch size的图像的转换矩阵
            ...
            if (
                ...
                num_dn_group, num_dn = dn_metas["dn_anchor"].shape[
                    1:3
                ]  # 时序的噪声群组是在5个中保留了3个
    ...
    def update(self, instance_feature, anchor, confidence):
        if self.cached_feature is None:
            return instance_feature, anchor  # 在第一帧时直接返回所有的query
        ...
        _, _, (selected_feature, selected_anchor) = topk(
            confidence, N, instance_feature, anchor
        )  # 根据900个confidence选择前300个最大，根据他们的indice选出instance feature和anchor
        selected_feature = torch.cat(
            [self.cached_feature, selected_feature], dim=1
        )  # 在单帧层，将上一帧缓存的top600个query和这一帧的单帧层的top300个query拼接
    ...
    def cache(
        ...
        if k_pred is not None:
            ...
            k_pred = torch.cat(
                [
                    k_pred,
                    k_pred.new_zeros(
                        k_pred.shape[0], 300, k_pred.shape[2], k_pred.shape[3]
                    ),
                ],
                dim=1,
            )
        else:
            k_pred = anchor.new_zeros(anchor.shape[0], 900, 4, 2)
        bs, num_anchor, dim1, dim2 = k_pred.shape
        k_pred = k_pred.reshape(bs, num_anchor, dim1 * dim2)
        ...
        self.cached_k_pred = self.cached_k_pred.reshape(bs, 600, dim1, dim2)
    ...
    def update_instance_id(self, instance_id=None, confidence=None):
        ...
        instance_id = topk(temp_conf, self.num_temp_instances, instance_id)[2][0]
