...
class SparseBox3DTarget(BaseTargetWithDenoising):
    ...
    def sample(
        ...
        box_target = self.encode_reg_target(
            box_target, box_pred.device
        )  # [x,y,z,w,l,h,sin,cos,vx,vy] 10维
    ...
    def get_dn_anchors(self, cls_target, box_target, gt_instance_id=None):
        ...
        if self.max_dn_gt > 0:  # 每个sample的实例数量不同，但只取32个，多的删，少的补
        ...
        )  # 给每个sample不足32时，填充位置向量到32个，填充位置向量为全0向量
        ...
        if self.add_neg_dn:
            ...
            dn_anchor = torch.cat(
                [dn_anchor, box_target + noise_neg], dim=1
            )  # 拼接正负噪声，各32个
            num_gt *= 2

        box_cost = self._box_cost(
            dn_anchor, box_target, torch.ones_like(box_target)
        )  # 5组*8个sample=40个（64(正负噪声框)*32(gt)的cost矩阵）
        dn_box_target = torch.zeros_like(dn_anchor)  # [40,64,10]
        dn_cls_target = (
            -torch.ones_like(cls_target) * 3
        )  # [40,32] 上面的回归考虑负噪声，这里的分类只考虑正噪声
        if gt_instance_id is not None:
            dn_id_target = -torch.ones_like(
                gt_instance_id
            )  # [40,32] 这里也只考虑正噪声
        if self.add_neg_dn:  # 但是当考虑负噪声时,上面两个[40,32]都要变为[40,64]
        ...
        for i in range(
            dn_anchor.shape[0]
        ):  # 给40个cost矩阵依次匈牙利，并给其中的64个噪声分配gt，只能分配到32个，理想情况下是正噪声这32个
        ...
        dn_anchor = (
            ...
            .permute(1, 0, 2, 3)
            .flatten(1, 2)
        )  # [40,64,10]->[5,8,64,10]->[8,5,64,10]->[8,320,10]
        dn_box_target = (
        ...
        if self.add_neg_dn:
            cls_target = (
                ...
                .permute(1, 0, 2)
                .flatten(1)
            )  # 这里的[cls_target, cls_target],主要是第二个cls_target要为下面一行((cls_target >= 0) & (dn_cls_target == -3))服务
            valid_mask = torch.logical_or(
                valid_mask, ((cls_target >= 0) & (dn_cls_target == -3))
            )  # valid 表示不是填充的
        attn_mask = (
            dn_box_target.new_ones(  # 参考DN-DETR笔记，不同组的query不能看到彼此
        ...
        return (
            ...
            dn_box_target,  # （正例噪声的要回归的gt位置，填补到32的正例噪声要回归的位置[零]，负例噪声要回归的位置[零]，填补到32的负例噪声要回归的位置[零]
            dn_cls_target,  # 最值得探究的一个:(正例噪声的类别，填补到32的正例噪声的类别[-1]，负例噪声的类别[-3]，填补到32的负例噪声的类别[-3]) * 5
            attn_mask,
    ...
    def update_dn(
        ...
        # update temp_dn_metas by instance_id 有三组缓存的噪声
        temp_dn_feat = self.dn_metas["dn_instance_feature"]  # [8,3,64,256]
        _, num_temp_dn_groups, num_temp_dn = temp_dn_feat.shape[:3]
        temp_dn_id = self.dn_metas["dn_id_target"]  # [8,3,64]

        # bs, num_temp_dn_groups, num_temp_dn, num_dn
        # 5组dn_query有3组是时序的，但是时序的到当前帧，两帧的id可能会变，所以将当前帧的dn_query的gt根据相同的id赋给时序的dn_query
        match = (
            temp_dn_id[..., None] == dn_id[:, :num_temp_dn_groups, None]
        )  # temp_dn_id是[8,3,64],dn_id是[8,5,64],因此在判断[8,3,64,1]==[8,3,1,64]
        # 这里重点理解,match的维度是[8,3,64,64],只看后两维，
        # [64,64]代表第一个和64个哪个相同的就是True、第二个和64个哪个相同的就是True，第64个和64个哪个相同的就是True
        temp_reg_target = (
            match[..., None] * dn_reg_target[:, :num_temp_dn_groups, None]
        ).sum(
            dim=3
        )  # [8,3,64,64,+1] * [8,3,+1,64,10],这里是点乘，不是叉乘。
        # 结果是[8,3,64,64,10],再sum(dim=3)后是[8,3,64(这里对应原来的第二个64),10]
        # 再来看点乘，只看[64,64,+1]*[+1,64,10]的部分，左边取一行，只有第i个值为true，在和右边的64个回归值相乘，最终只有第i个回归值被保留
        ...
        # 当match维度[8,3,64,64]的[64,64]中,第n行全零，则第n行没有匹配到，根据代码逻辑置类别为-1
        ...
        temp_dn_metas = [  # 这里的维度都是[8,3,64]打头
            temp_dn_feat,
            temp_dn_anchor,
            temp_reg_target,
            temp_cls_target,
        ...
        for i, (temp_meta, meta) in enumerate(zip(temp_dn_metas, dn_metas)):
            ...
            meta = torch.cat(
                [temp_meta, meta[:, num_temp_dn_groups:]], dim=1
            )  # 将3组缓存的噪声query和2/5组实例噪声query拼接
            meta = meta.flatten(1, 2)
            output.append(meta)
        output[0] = torch.cat([instance_feature, output[0]], dim=1)  # 900+320
        output[1] = torch.cat([anchor, output[1]], dim=1)  # 900+320
        return output
