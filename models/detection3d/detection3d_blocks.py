...
from .transformer_k_encoder import *
...
class KalmanEncoder(BaseModule):
    def __init__(self, input_dims=256, output_dims=16, transformer_head=True):
        super().__init__()
        self.input_dims = input_dims
        self.output_dims = output_dims
        self.cached_k_encoder = nn.Sequential(*linear_relu_ln(256, 2, 2, output_dims))
        self.ego_pose_k_encoder = nn.Sequential(*linear_relu_ln(256, 2, 2, 16))
        self.transformer_k_encoder = Transformer_k_encoder()
        self.transformer_head = transformer_head
        self.k_encoder_fc = nn.Sequential(
            *linear_relu_ln(input_dims, 2, 2),
            Linear(input_dims, output_dims),
            Scale([1.0] * output_dims),
        )

    def forward(
        self,
        before_align_anchor_embed,
        after_align_anchor_embedding,
        anchor_embed,
        cached_k_pred,
        ego_pose,
    ):
        if cached_k_pred.shape[-1] == self.output_dims:
            cached_k_pred = self.cached_k_encoder(cached_k_pred)
        ego_pose_embed = self.ego_pose_k_encoder(
            ego_pose.reshape(ego_pose.shape[0], -1)
        )
        if self.transformer_head:
            return self.k_encoder_fc(
                self.transformer_k_encoder(
                    before_align_anchor_embed,
                    after_align_anchor_embedding,
                    anchor_embed,
                    cached_k_pred,
        ...
        k = self.k_encoder_fc(concat_embed)
        return k
    ...
    def anchor_projection(
        ...
        for i in range(len(T_src2dst_list)):
            ...
            if time_interval is not None:
                translation = (
                    vel.transpose(0, -1) * time_interval
                )  # 时序的query的速度乘时间间隔得到位移
                translation = translation.transpose(0, -1)
                center = center - translation  # 将位移加到query的中心上
            center = (
                torch.matmul(T_src2dst[..., :3, :3], center[..., None]).squeeze(dim=-1)
                + T_src2dst[..., :3, 3]
            )  # 自车运动对query的补偿，通过对query的中心乘自车的帧间位姿平移矩阵加旋转矩阵
            size = anchor[..., [W, L, H]]
            yaw = torch.matmul(
                T_src2dst[..., :2, :2],
