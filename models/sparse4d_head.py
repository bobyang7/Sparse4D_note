...
class Sparse4DHead(BaseModule):
    ...
    def forward(
        ...
        if self.instance_bank.cached_anchor is not None:
            before_align_anchor_embed = self.anchor_encoder(
                self.instance_bank.cached_anchor
            )
        ...
        if temp_anchor is not None:  # 第一帧的时候没有temp_anchor
            ...
            after_align_anchor_embed = temp_anchor_embed
        else:
        ...
        for i, op in enumerate(self.operation_order):
            if self.layers[i] is None:
                ...
                ego_pose = anchor_embed.new_tensor(
                    [i["T_global"] for i in metas["img_metas"]]
                )
                ego_pose = torch.tile(ego_pose[:, None], (1, 600, 1, 1))
                if temp_anchor is not None:
                    cached_k_pred = self.instance_bank.cached_k_pred
                    mask_long = torch.logical_not(
                        torch.all(
                            cached_k_pred.reshape(
                                cached_k_pred.shape[0],
                                600,
                                cached_k_pred.shape[2] * cached_k_pred.shape[3],
                            )
                            == 0,
                            dim=-1,
                        )
                    )
                    mask_short = torch.logical_not(mask_long)
                    k_pred = torch.zeros_like(cached_k_pred)
                    if mask_short.sum():
                        k_pred_short = self.layers[i](
                            before_align_anchor_embed[mask_short],
                            after_align_anchor_embed[mask_short],
                            anchor_embed[:, :600][mask_short],
                            anchor_embed[:, :600][mask_short],
                            ego_pose[mask_short],
                        )
                        k_pred[mask_short] = k_pred_short.reshape(
                            mask_short.sum(), 4, 4
                        )
                    if mask_long.sum():
                        k_pred_long = self.layers[i](
                            before_align_anchor_embed[mask_long],
                            after_align_anchor_embed[mask_long],
                            anchor_embed[:, :600][mask_long],
                            cached_k_pred[mask_long].reshape(mask_long.sum(), -1),
                            ego_pose[mask_long],
                        )
                        k_pred[mask_long] = k_pred_long.reshape(mask_long.sum(), 4, 4)
                    else:
                        useless2 = (
                            self.layers[i](
                                anchor_embed[:, :600].flatten(end_dim=1),
                                anchor_embed[:, :600].flatten(end_dim=1),
                                anchor_embed[:, :600].flatten(end_dim=1),
                                anchor_embed.new_zeros(
                                    anchor_embed.shape[0], 600, 16
                                ).flatten(end_dim=1),
                                ego_pose.flatten(end_dim=1),
                            )
                            * 0
                        ).reshape(anchor_embed.shape[0], 600, 4, 4)
                        k_pred = k_pred + useless2
        ...
        self.instance_bank.cache(
            ...
            anchor,
            # anchor_embed,
            k_preds[-1] if len(k_preds) else None,
            cls,
            metas,
            feature_maps,
        )
    ...
    def loss(self, model_outs, data, feature_maps=None):
        ...
        for decoder_idx, (cls, reg, qt) in enumerate(
            ...
            # reg_target是匹配的对象真值，匹配到的里面的属于前600个的reg_target先拿出来
            if self.kal and decoder_idx == len(reg_preds) - 1 and xt is not None:
                if (
                    self.instance_bank.mask is None
                    or sum(self.instance_bank.mask) == reg.shape[0]
                ):
                    mask_temp = mask.reshape(8, 900)[:, :600].reshape(-1)
                    zt = reg[:, :600, [0, 1, 8, 9]].flatten(end_dim=1)[mask_temp]
                    xt_ = xt[..., [0, 1, 8, 9]].flatten(end_dim=1)[mask_temp]
                    kt_pred = kt_preds[-1].flatten(end_dim=1)[mask_temp]
                else:
                    mask_temp = mask.reshape(8, 900)[:, :600][
                        self.instance_bank.mask
                    ].reshape(-1)
                    zt = reg[:, :600, [0, 1, 8, 9]][self.instance_bank.mask].flatten(
                        end_dim=1
                    )[mask_temp]
                    xt_ = xt[..., [0, 1, 8, 9]][self.instance_bank.mask].flatten(
                        end_dim=1
                    )[mask_temp]
                    kt_pred = kt_preds[-1][self.instance_bank.mask].flatten(end_dim=1)[
                        mask_temp
                    ]

                h_xt = xt_[..., :4]
                update_xt = xt_[:, :, None] + torch.matmul(
                    kt_pred, (zt - h_xt)[:, :, None]
                )
                mask_fill = mask.clone().reshape(8, 900)
                mask_fill[:, 600:] = 0
                mask_fill[torch.logical_not(self.instance_bank.mask)] = 0
                update_xt_ = update_xt.new_zeros(8, 900, 4)
                update_xt_[mask_fill] = update_xt.squeeze(-1)
                update_reg = torch.where(
                    mask_fill[..., None], update_xt_, reg[..., [0, 1, 8, 9]]
                )
                update_reg = torch.cat(
                    [update_reg[..., :2], reg[..., 2:8], update_reg[..., 2:]], dim=-1
                )
                # 然后更新self.instance.cache_anchor,利用self.instance.index
                # self.instance_bank.cached_anchor = torch.cat(
                #     [
                #         update_reg.flatten(end_dim=1)[
                #             self.instance_bank.indices
                #         ].reshape(reg.shape[0], 600, -1),
                #         reg_vz.flatten(end_dim=1)[self.instance_bank.indices].reshape(
                #             reg.shape[0], 600, -1
                #         ),
                #     ],
                #     dim=-1,
            ...
            update_reg = (
                update_reg.flatten(end_dim=1)[mask]
                if self.kal and decoder_idx == len(reg_preds) - 1 and xt is not None
                else reg.flatten(end_dim=1)[mask]
            )
            # update_reg = reg.flatten(end_dim=1)[mask]

            # ls_xy_sum = []
            # ls_vxvy_sum = []
            # for i in range(reg_target.shape[0]):
            #     ls_xy = abs(reg_target[i][0] - update_reg[i][0]) + abs(
            #         reg_target[i][1] - update_reg[i][1]
            #     )
            #     ls_xy_sum.append(ls_xy)
            #     ls_vxvy = abs(reg_target[i][8] - update_reg[i][8]) + abs(
            #         reg_target[i][9] - update_reg[i][9]
            #     )
            #     ls_vxvy_sum.append(ls_vxvy)
            #     # print(
            #     #     f"tar_xy: {reg_target[i][:2]}, tar_vxvy:{reg_target[0][8:10]}, ls_xy: {ls_xy}, ls_vxvy: {ls_vxvy}"
            #     # )
            # print(
            #     "=" * 50
            #     + f" total: {reg_target.shape[0]}, loss_xy_sum: {np.sum(ls_xy_sum)}, loss_vxvy_sum: {np.sum(ls_vxvy_sum)}"
            # )
            ...
            # if decoder_idx == len(reg_preds) - 1:
            #     print(f"loss_box_5: {reg_loss['loss_box_5']}")

            # hotfix
            # if self.kal and decoder_idx == len(reg_preds) - 1 and xt is not None:
            #     origin_reg = reg.flatten(end_dim=1)[mask]
            #     origin_reg_loss = self.loss_reg(
            #         origin_reg,
            #         reg_target,
            #         weight=reg_weights,
            #         avg_factor=num_pos,
            ...
            #     )
            #     print(
            #         f"origin_loss_box_5: {origin_reg_loss['loss_box_5']},      update_loss_box_5: {reg_loss['loss_box_5']}"
            #     )

            if xt is None and self.kal and decoder_idx == len(reg_preds) - 1:
                cls_loss = cls_loss + kt_preds[-1].sum()
        ...
        for decoder_idx, (cls, reg) in enumerate(zip(dn_cls_scores, dn_reg_preds)):
            ...
            output.update(reg_loss)
        # print(f"l5_reg_loss:{output['loss_box_5']}, l4_reg_loss:{output['loss_box_4']}")
        return output
